...
def evaluate_move(board, piece_type, move, debug=False):
    """
    ...
    """
    i, j = move
    win_s = win_score(board, piece_type, move)
    kill_s = kill_enemy_score(board, piece_type, move)
    kill_s2 = kill_enemy_score2(board, piece_type, move)
    be_killed_s = be_killed_score(board, piece_type, move)
    liberty_s = liberty_score(board, piece_type, move)
    connection_score = connecting_score(board, piece_type, move)
    edge_s = edge_score(board, move)
    # if debug:
...
def win_score(board, piece_type, move):
    """
    ...
    s = score(test_board, piece_type) - score(test_board, 3 - piece_type)
    if piece_type == 2:
        s += 2.5
    return s


def kill_enemy_score(board, piece_type, move):
    """
...
def kill_enemy_score2(board, piece_type, move):
    """
    ...
    test_board = deepcopy(board)
    test_board[i][j] = piece_type
    valid_moves = get_valid_moves(board, test_board, piece_type)
    for move in valid_moves:
        kill_cnt = kill_enemy_score(test_board, piece_type, move)
        if kill_cnt > 0:
            return kill_cnt
...
def be_killed_score(board, piece_type, move):
    """
    ...
    new_board = deepcopy(board)
    new_board[i][j] = piece_type
    valid_moves = get_valid_moves(board, new_board, 3-piece_type)
    for k, l in valid_moves:
        test_board = deepcopy(new_board)
        test_board[k][l] = 3-piece_type
        died_pieces = find_died_pieces(test_board, piece_type)
...
def liberty_score(board, piece_type, move):
    """
    ...
    after_liberty = count_liberty(after_board, piece_type)
    return after_liberty


def edge_score(board, move):
    """
...
def connecting_score(board, piece_type, move):
    """
