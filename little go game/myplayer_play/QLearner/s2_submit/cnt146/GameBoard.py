...
def detect_neighbor_all(board_int, i, j):
    ...
    :return: a list containing the neighbors row and column
    """
    neighbors = []
    if i > 0:
...
def detect_neighbor(board_int, i, j):
    """
    Detect neighbors of a given stone in 4 directions
...
def detect_neighbor_ally(board_int, i, j):
    ...
    :return: a list containing the neighbored allies row and column
    """
    ...
    for nei_i, nei_j in neighbors:
        ...
        if get_board_piece(board_int, nei_i, nei_j) == get_board_piece(board_int, i, j):
            group_allies.append((nei_i, nei_j))
    return group_allies
...
def ally_dfs(board_int, i, j):
    ...
    """
    stack = [(i, j)]  # stack
    result = []  # allies positions
    while stack:
        piece = stack.pop()
        result.append(piece)
        neighbors = detect_neighbor_ally(board_int, piece[0], piece[1])
        for nei in neighbors:
            if nei not in stack and nei not in result:
                stack.append(nei)
    return result
...
def find_died_pieces(board_int, piece_type):
    """
    find the died piece that has no liberty
...
def remove_certain_pieces(board_int, positions):
    ...
    :return: new board after removal
    """
...
def remove_died_pieces(board_int, piece_type):
    ...
    :param piece_type: 1('X') or 2('O').
    :return: (list of dead pieces, new board)
    """
...
def evaluate_move(board_int, piece_type, move, debug=False, params=None):
    """
    ...
    kill_eye_s = kill_eye_score(board_int, piece_type, move)
    # if debug:
    #     print("on move ({},{}) win: {} kill: {} liberty: {} connection: {} edge: {}".format(
    #         i, j, win_score, kill_score, liberty_score, connection_score, edge_score))
    return win_s * 0.3 + kill_s * 8 + kill_s2 * 2 + be_killed_s * 10 + liberty_s * 0.1 + connection_score * 2 + edge_s * 0.1 + kill_eye_s * 10
...
def kill_eye_score(board_int, piece_type, move):
    i, j = move
    all_nei = detect_neighbor_all(board_int, i, j)
    find_enemy = False
    find_empty = False
    for nei in all_nei:
        p = get_board_piece(board_int, nei[0], nei[1])
        if p == 3 - piece_type:
            find_enemy = True
        elif p == 0:
            find_empty = True
    if not find_enemy and not find_empty:
        return -1
    else:
        return 0
