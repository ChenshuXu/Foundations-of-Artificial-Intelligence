...
class MinmaxPlayer:
    ...
    def get_input(self, piece_type, previous_board, board):
        ...
        score, action = self._max(previous_board, board, "move")
        if self.debug:
    ...
    def _max(self, previous_board, board, previous_action):
        ...
        for i, j in valid_moves:
            ...
            if not new_board:
                print("empty")
            score = self._minmax(False, new_previous_board, new_board, "move", self.depth-1, float("-inf"), float("inf"), debug=False)
            if self.debug:
    ...
    def _minmax(self, is_max, previous_board, board, previous_action, depth, alpha, beta, debug=False):
        if is_max:
        ...
        valid_moves = self._get_valid_moves(previous_board, board, piece_type)
        ...
        if depth < 1:
            for move in self._get_valid_moves(previous_board, board, self.piece_type):
                score = self._evaluate_move(board, self.piece_type, move)
                if is_max:
        ...
        for i, j in valid_moves:
            new_previous_board = deepcopy(board)
            # impossible to be unsuccessful
            new_board, success = GameBoard.place_chess(previous_board, board, i, j, piece_type)
            score = self._minmax(not is_max, new_previous_board, new_board, previous_action, depth - 1, alpha, beta)
            if is_max:
                best_value = max(score, best_value)
    ...
    def _evaluate_board(self, previous_board, board, piece_type, debug=False):
        ...
        """
        score = GameBoard.score(board, piece_type) - GameBoard.score(board, 3 - piece_type)
        if piece_type == 2:
            score += 2.5
        return score

    def _evaluate_board2(self, previous_board, board, piece_type, debug=False):
        """
        evaluate score of this board
        """
        best_value = float("-inf")
